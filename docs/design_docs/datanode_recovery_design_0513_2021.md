...
DataNode is based on flowgraph; each flowgraph cares about only one vchannel. There are data definition language (DDL) messages, dml
messages, and timetick messages inside one vchannel, FIFO log stream.

One vchannel only contains data manipulation language (DML) messages of one collection. A collection consists of many segments, hence
